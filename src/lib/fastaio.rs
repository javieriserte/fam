...
pub fn sequence_collection_from_bufread<T: BufRead>(mut reader: T) -> Result<SequenceCollection, Error> {
...
pub fn sequence_collection_from_file(path: &Path) -> Result<SequenceCollection, Error> {
...
pub fn write_sequence_collection<T1: SequenceAccesors, T2: Write>(seqs: T1, writer: T2) -> Result<(),io::Error>{
    // let path = "hola.txt";
